...
const Register = () => {
    ...
    const register = (evt) => {
        ...
        if(username.trim() === '' || password.trim() === '') {
            ...
            axios.post("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/auth/register", {username, password}, { withCredentials: true })
            .then((user) => {
                const status = user.data;
                if(status === 'success') {
                    axios.post("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/auth/login", {username, password}, { withCredentials: true })
                        .then(() => navigate('/'));
                } else {
                    setRegisterError('badLogin');
                }
