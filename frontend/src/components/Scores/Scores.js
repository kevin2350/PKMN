...
const Scores = () => {
    ...
    const log = (evt) => {
        evt.preventDefault();
        axios.get("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/auth/user", { withCredentials: true })
            .then((user) => {
                const status = user.data;
                if(status !== 'unlogged') {
                    axios.get("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/auth/logout", { withCredentials: true })
                        .then((res) => {
                            setLogged('Unlogged');
                            setAction('Login');
                            setUsername('');
    ...
    const removeEntry = (evt) => {
        evt.preventDefault();
        axios.post("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/scores/remove", {id: scores[evt.target.value].id})
        ...
        axios.get("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/auth/user", { withCredentials: true })
            .then((user) => {
                ...
                if(name !== 'unlogged') {
                    ...
                    axios.get("http://ec2-3-128-25-22.us-east-2.compute.amazonaws.com:3001/scores")
                        .then((res) => {
                            const data = res.data;
                            data.map((score) => {
